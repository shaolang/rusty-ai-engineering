...
async fn main() {
    ...
    loop {
        ...
        cprintln(Red, &format!("Assistant: {}", remove_bracket_tags(&response)));
    }
}

fn remove_bracket_tags(text: &str) -> String {
...
fn system_prompt(rag_chunk: Option<String>) -> String {
    ...
    format!(
        ...
        * If the user doesn't mention the name of which GROSS software they're asking about,
          proactively ask them which software they're using.
        * When asking proactive follow-up questions, ask exactly one question at a time.
